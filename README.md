...
const request: Forum.PostAuthRegisterRequest = {
    ...
};
```
...
```typescript
const { data, rawResponse } = await client.auth.register(...).withRawResponse();
...
```typescript
import { ForumClient, logging } from "@foru-ms/sdk";
